...
class UserPanelController extends Controller
{
    ...
    function UserprofileSetting(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), ['image' => 'mimes:jpeg,bmp,png']);
        if ($validator->fails()) {
            return response()->json(['msg' => 2, 'error' => $validator->errors()->all()]);
        }
        $is_file = $is_banner = false;
        ...
        if ($user) {
            ...
            if ($request->hasFile('banner')) {
                $banner_name = $input['banner'] = uniqid() . time() . '.' . $request->banner->getClientOriginalExtension();
                $request->banner->move(base_path() . '/assets/images/banner/', $input['banner']);
