...
class AjaxController extends Controller
{
    ...
    {
        $data = User::All()->whereNotIn('type', ['adm', 'c']);
    ...
    function changeStatus(Request $request){
        ...
        if ($object == 'auction')
        {
            $stat = Auction::where(['ad_id' => $request->id])->value('status');
            if ($stat == 0) { $status = 1; }
            if ($stat == 1) { $status = 0; }

            $update = Auction::where(['ad_id' => $request->id])->update(['status' => $status]);
            if ($update)
            {
                echo json_encode(array('res' => $status));
            }
        ...
        if ($object == 'users_company') {
            $stat = User::where(['id' => $request->id])->value('preview');
            if ($stat == 2) {
                $status = 1;
            }
            if ($stat == 1) {
            ...
            $update = User::where(['id' => $request->id])->update(['preview' => $status]);
            if ($update) {
                echo json_encode(array('res' => $status));
            }
        }
    ...
    public function loadShowrooms(Request $request)
    {
        $data = User::All()->where('type', 'c');
        return Datatables::of($data)->editColumn('banner', function ($data) {
            $banner = '';
            if ($data->banner == '') {
                $banner .= "<img src='" . asset('assets/images/banner/empty.png') . "' class='img-responsive'>";
            } else {
                $banner .= "<img src='" . asset('assets/images/banner/' . $data->banner) . "' class='img-responsive'>";
            }
            return $banner;
        })->editColumn('email', function ($data) {
            $email = '';
            ...
            if ($data->preview == 0) {
                $b .= '&nbsp;<a  title="Company haven\'t requested for showroom activation" class="btn btn-xs m-t-10 btn-purple ">No Request</a>';
            }
            if ($data->preview == 1) {
                $b .= '&nbsp;<a onclick="changeStatus(this)" data-obj="users_company" data-id="' . $data->id . '"  title="Company haven\'t requested for showroom activation" class="btn btn-xs m-t-10 btn-success ">Company Activated</a>';
            }
            if ($data->preview == 2) {
                $b .= '&nbsp;<a onclick="changeStatus(this)" data-obj="users_company" data-id="' . $data->id . '"  title="Company requested for showroom activation | click to activate" class="btn btn-xs m-t-10 btn-purple ">Activation Requested</a>';
            }
            return $b;
        })->rawColumns(['email', 'action', 'banner'])->make(true);
    }
    ...
    function loadMyAds(Request $request)
    {
        ...
        return Datatables::of($data)
            ...
            ->editColumn('title', function ($data) {
                $title = '';
                $title .= '<a href="'.url('single/'.urlencode(strtolower(str_slug( $data->title.'-'.$data->id, '-')))  ).'">'.ucfirst($data->title).'</a>';
                return $title;
            })
            ->addColumn('auction', function ($data) {
                $b='No';
                if( Auction::where('ad_id', $data->id)->exists() ){
                    $b = 'Yes';
                }
                return $b;
            })
            ->addColumn('action', function ($data) use ($request) {
                ...
                $b ='';
                if($data->status != 3){
                  $b .= '<a href="'.route('ads.edit', $data->id).'" title="Edit" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a> ';
                }
                $b .= '&nbsp;<a onclick="deleteRow(this)" data-id="' . $data->id . '" data-obj="user_ads" href="javascript:;" title="Delete" class="btn btn-sm btn-danger danger-alert"><i class="glyphicon glyphicon-trash"></i></a> ';
                if($data->status == 1 || $data->status == 2)
                {
                    $b .= '<a onclick="changeStatus(this)" data-obj="user_ads" data-id="' . $data->id . '"  title="'.$title.'" class="btn btn-sm '.$btn_clr.'"><i class="fa '.$status.'"></i></a>';
                }
                if($data->status == 3)
                {
                ...
                if($request->load_ads == 'all') {
                    if (Auction::where('ad_id', $data->id)->exists()) {
                        $b .= '&nbsp;<a onclick="set_auction(this)" data-id="' . $data->id . '" data-price="' . $data->price . '" data-t="' . $data->title . '" title="Available for auction" class="btn btn-xs btn-success m-t-5"><i class="fa fa-gavel "></i></a>';
                    } else {
                        $b .= '&nbsp;<a onclick="set_auction(this)" data-id="' . $data->id . '" data-price="' . $data->price . '" data-t="' . $data->title . '" title="Set for auction" class="btn btn-xs btn-warning m-t-5"><i class="fa fa-gavel "></i></a>';
                    }
                }
    ...
    */
    ...
    function loadAuctionAds(Request $request)
    {

        $ad_ids = Auction::where('user_id', auth()->user()->id )->pluck('ad_id');
        ...
        return Datatables::of($data)
            ...
            ->editColumn('region_title', function ($data) {
                $d = $data->region_title.'/'.$data->city_title;
                return $d;
            })

            ->addColumn('top_bid', function ($data) {
                $auction_id = Auction::where('ad_id', $data->ads_id)->value('id');
                $max_bid = UserBids::where('auction_id',$auction_id )->max('amount');
                return $max_bid==''? 0: $max_bid;
            })
            ->editColumn('title', function ($data) {
                $title = '';
                $title .= '<a href="'.url('single/'.urlencode(strtolower(str_slug($data->title.'-'.$data->ads_id, '-')))  ).'">'.ucfirst($data->title).'</a>';
            ...
            ->addColumn('action', function ($data) {
                $b='';
                $auction = Auction::where('ad_id', $data->ads_id)->first();
                if($auction->status==1) {
                    $b = '&nbsp;<a onclick="changeStatus(this)" data-obj="auction" data-id="' . $data->ads_id . '"  title="Ad activated for auction" class="btn btn-xs btn-success "><i class="fa fa-unlock "></i></a>';
                }else {
                    $b = '&nbsp;<a onclick="changeStatus(this)" data-obj="auction" data-id="' . $data->ads_id . '"  title="Ad not activated for auction" class="btn btn-xs btn-warning "><i class="fa fa-lock "></i></a>';
                }
                $b .= '&nbsp;<button title="Bid history" class="btn btn-xs btn-warning" onclick="bid_history(' . $data->ads_id . ')"><i class="fa fa-bar-chart"></i></button>';
    ...
    function get_my_bids(Request $request)
    {

        $data = DB::table('ads')
            ->join('auctions', 'auctions.ad_id', 'ads.id')
            ->join('user_bids', 'auctions.id', 'user_bids.auction_id')
            ->join('city', 'city.id', 'ads.city_id')
            ->join('categories', 'categories.id','ads.category_id')
            ->join('users', 'users.id','ads.user_id')
            ->select(
            ...
            ->where('user_bids.user_id', auth()->user()->id);
        ...
        return Datatables::of($data)
            ...
            ->addColumn('status', function ($data) {
                $b='';
                if($data->won==1) {
                    $b = '<span class="text-success">Accepted <i class="fa fa-check "></i></span>';
                }else if($data->won==2) {
                    $b = '<span class="text-danger">Rejected <i class="fa fa-times "></i></span>';
                }
                else if($data->won==0) {
                    $b = '<span class="text-info">Pending</span>';
                }
                return $b;
            })
        ...
        $filename =getcwd() . '/watch.csv';
            ...
            if(is_file($filename)) {
                ...
                if (($handle = fopen($filename, 'r')) !== false) {
                    ...
                    foreach ($dataArray as $ind => $value){
                       /*$parent_id = Category::insertGetId(['parent_id' => 3, 'name' => $ind, 'slug' => str_slug($ind, '-')]);
                       foreach ($value as $child){
                           Category::insert(['parent_id' => $parent_id, 'name' => $child, 'slug' => str_slug($child, '-')]);
                       }*/
                    }
