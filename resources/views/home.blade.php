        ...
        <div class="banner-section text-center">
            ...
            <div class="banner-form">
                <form action="{{url('search/query')}}" method="get">
                    <!-- category-change -->
                    <input style="border-right:1px solid #bfbfbf !important" type="text" class="form-control" name="keyword" placeholder="Type Your key word">
                    <input type="text" id="address-input" class="form-control locinput input-rel searchtag-input has-icon selecters" placeholder="City">
                    {{--<select name="city" id="region" class="form-control locinput input-rel searchtag-input has-icon selecters">
                        <option value="">Select City</option>
                        @foreach($city as $value)
                            <option value="{{ $value->id }}">{{ ucfirst($value->title) }}</option>
                        @endforeach
                    </select>--}}
                    <select name="category" style="height: 48px;
    border-left: 1px solid #bfbfbf !important;
    border-radius: 0;
    margin-left: 1px;" class="form-control locinput input-rel has-icon" id="category">
...
<section style="padding: 25px 0 57px;
            ...
            <div class="">
                @foreach($regionAds as $val)
                    <div class=" col-md-2 col-sm-3 col-xs-6">
                        <a class="btn btn-sm btn-default btn-block m-b-5" href="{{ url('search/query') }}?city={{ $val->title }}"> <span class="text-right pull-left"> {{$val->title}}</span> <span class="badge pull-right">{{ \App\Ads::where(['city_id' => $val->id, 'status' => 1])->count() }}</span> <span class="clearfix"></span></a>
                    </div>
                @endforeach
            </div>
        </div>
